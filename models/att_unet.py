...
class AttUnet(Model):
    ...
    def learning(self, x, y, test_x, test_y):
        ...
        cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath='weights/att_unet_maxmin.ckpt',
        ...
        self.fit(x, y, validation_data=(test_x, test_y), epochs=1000, batch_size=1,
...
class ssim_metric(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        self.ssim = tf.image.ssim(y_true, y_pred, max_val=1)
...
class psnr_metric(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        self.psnr = tf.image.psnr(y_true, y_pred, max_val=1)
